...
func (cq *CategoriesQuery) QueryProducts() *ProductsQuery {
	query := &ProductsQuery{config: cq.config}
...
func (cq *CategoriesQuery) WithProducts(opts ...func(*ProductsQuery)) *CategoriesQuery {
	query := &ProductsQuery{config: cq.config}
...
func (cq *CategoriesQuery) sqlAll(ctx context.Context) ([]*Categories, error) {
	...
	if query := cq.withProducts; query != nil {
		...
		for i := range nodes {
			...
			nodes[i].Edges.Products = []*Products{}
		}
		query.Where(predicate.Products(func(s *sql.Selector) {
			s.Where(sql.InValues(categories.ProductsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			...
			node, ok := nodeids[fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "categorie_id" returned %v for node %v`, fk, n.ID)
			}
